-- generate_series expands to a full `with ... select` (a generated_number
-- column 1..N), so it has to sit as a CTE body; subtract 1 for 0-based
with days as (
...
select
    ...
    order_day_of_week,
    -- Instacart never documented the dow mapping; 0/1 are usually taken as
    -- the weekend based on order-volume patterns, so this is an assumption
